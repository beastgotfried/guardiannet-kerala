...
import { Shield, Menu, Github, ExternalLink, X, Users, Briefcase, Zap, Lock, AlertTriangle } from "lucide-react";
          ...
          {showPrivacy && (
            ...
            >
              ...
              <motion.div 
                ...
                animate={{ scale: 1, y: 0 }}
                className="relative w-full max-w-2xl glass-card p-8 md:p-12 rounded-[3rem] border-primary/20 shadow-[0_0_100px_rgba(var(--primary),0.1)] overflow-hidden"
              >
                ...
                <div className="flex items-center gap-4 mb-8">
                  ...
                  <div>
                    <h2 className="text-3xl font-black italic tracking-tight">GuardianNet Privacy Notice</h2>
                    <p className="text-foreground/40 text-xs font-mono uppercase tracking-widest">Data Protection & Safety Protocol</p>
                  </div>
                </div>

                <div className="space-y-6 text-foreground/70 leading-relaxed max-h-[40vh] overflow-y-auto pr-4 custom-scrollbar mb-10">
                  <p>
                    GuardianNet collects only the minimum information required to support disaster preparedness and emergency response. This may include basic identification details, declared skills or assets, and location data during active disaster events or simulations. We do not collect data for advertising, tracking, or commercial purposes.
                  </p>
                  <p>
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  className="w-full py-6 rounded-2xl bg-primary text-primary-foreground font-black text-xl shadow-2xl shadow-primary/20 hover:shadow-primary/40 transition-all uppercase tracking-widest"
                >
                  Agree and Continue
                </motion.button>
              </motion.div>
          ...
          <div className="container mx-auto px-4 h-20 flex items-center justify-between">
            ...
            >
              ...
              <div className="flex flex-col">
                ...
                <span className="hidden sm:inline text-[9px] text-foreground/40 font-mono">Kerala Edition</span>
              </div>
            </motion.div>
            ...
            <div className="hidden md:flex items-center gap-1">
              {[
                ...
                <motion.a 
                  ...
                  className={`px-4 py-2 rounded-lg text-sm font-medium text-foreground/60 hover:text-foreground hover:bg-white/5 transition-all whitespace-nowrap ${isMalayalam ? 'text-xs' : ''}`}
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                >
                  {item.name}
              ...
              <div className="flex items-center gap-1 mr-4 bg-white/5 rounded-xl p-1 border border-white/5 shrink-0">
                {[
                  { code: 'en', label: 'EN' },
                  { code: 'hi', label: 'HI' },
                  { code: 'ml', label: 'ML' }
              ...
              <motion.button 
                ...
                className={`px-5 py-2.5 rounded-xl bg-primary text-primary-foreground font-semibold text-sm flex items-center gap-2 shadow-lg shadow-primary/20 hover:shadow-primary/40 transition-all shrink-0 ${isMalayalam ? 'text-xs' : ''}`}
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
              >
                {t.nav.volunteer}
                      ...
                      <div className="flex-1 w-full">
                        ...
                        <div className="flex flex-wrap gap-4">
                          ...
                          <motion.button
                            ...
                            whileTap={{ scale: 0.95 }}
                            className={`px-8 py-3 rounded-xl bg-primary text-primary-foreground text-sm font-black flex items-center gap-3 shadow-lg shadow-primary/20 ${isMalayalam ? 'text-xs' : ''}`}
                          >
                          ...
                          <div className="flex flex-col gap-2 p-4 bg-red-500/5 border border-red-500/10 rounded-2xl">
                            <span className={`text-[10px] font-black ${isIndic ? '' : 'uppercase tracking-tighter'} text-red-500/60 mb-1`}>
                      ...
                      <AnimatePresence>
                        {showVolunteerInfo && (
                          <motion.div 
                            ...
                            exit={{ opacity: 0, x: 20 }}
                            className="flex-[1.5] grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 w-full"
                          >
                            {ZONES.map((item) => (
                              <motion.div 
                                key={item.id} 
              ...
              <div className="container mx-auto px-4 relative z-10">
                <div className="flex flex-col lg:flex-row justify-between items-start gap-16 mb-20">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-12 lg:gap-16">
                      <div>
                        <h4 className={`font-bold mb-6 text-sm ${isIndic ? '' : 'uppercase tracking-widest'} text-foreground/60`}>{t.footer.platform}</h4>
                        <ul className="space-y-4 text-sm text-foreground/40">
                          {t.footer.platformItems.map((item) => {
                            ...
                            // We match by name for simplicity as requested, but using translations
                            if (item === t.footer.platformItems[0]) { // Asset Mapping
                              href = "https://sdma.kerala.gov.in/hazard-maps/";
                              onClick = (e) => {
                                e.preventDefault();
                                handleExternalLink(href);
                              };
                            } else if (item === t.footer.platformItems[1]) { // Mesh Networking
                    ...
                    <div>
                      <h4 className={`font-bold mb-6 text-sm ${isIndic ? '' : 'uppercase tracking-widest'} text-foreground/60`}>{t.footer.regions}</h4>
                      <ul className="space-y-4 text-sm text-foreground/40">
                    ...
                    <div>
                      <h4 className={`font-bold mb-6 text-sm ${isIndic ? '' : 'uppercase tracking-widest'} text-foreground/60`}>{t.footer.resources}</h4>
                      <ul className="space-y-4 text-sm text-foreground/40">
                        <li>
                          <a 
